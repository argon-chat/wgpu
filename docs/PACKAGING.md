...
package's own major moves with it; a new **minor** over the same generation is an ordinary bump. The
watcher reports them as two different events — a week of work and an afternoon.

**Dawn is tracked by the calendar.** It tags every commit, roughly weekly, with no semver and no LTS
line, so "is there something newer" answers *yes* forever. The policy is to adopt whatever is current
**every six months**, measured from the last adoption.

Both live in [`upstream.manifest.ts`](../upstream.manifest.ts) as declared policies; the workflow
only evaluates them. ⚠ The schedule is a **daily** run (`cron: "10 6 * * *"`), not a cron per policy:
one rate-limited run of a twice-yearly cron and the next attempt is six months away, with nothing
saying so. Asking daily and answering by policy makes a missed run a one-day delay.
...
exactly the matching one. esbuild, swc, sharp and `bun-webgpu` all package their native halves this
way.

Each carries **two** shared libraries: upstream's `wgpu_native`, and this project's ABI shim. One
tarball rather than two packages, because a shim transcribes one wgpu-native generation's struct
layouts by hand and is only correct paired with it — the binding checks for skew at load and refuses,
...
only packages on its curated allowlist (`bun pm default-trusted`) run them, and anything else needs
*the consumer* to add this package to `trustedDependencies` or run `bun pm trust`.

Bun does say so — `bun install` prints `Blocked 1 postinstall. Run 'bun pm untrusted' for details.` —
but it still reports overall success, and the package is then broken at runtime with a
library-not-found error one easily-missed install line away from its cause. For a package whose
primary audience *is* Bun users, broken-by-default on Bun is not shippable.
...
**`optionalDependencies` fail silently.** On a platform with no matching package, your package
manager installs nothing and reports success. The first thing that notices is this package's own
resolver, so its error names the platform it looked for, the package that would have provided it, and
the `WGPU_NATIVE_LIB` override.

Resolution order is `WGPU_NATIVE_LIB` → `@wgpu-bun/<rid>` → `vendor/<rid>/`, so an explicitly built
library always wins and a development checkout works with no publishing at all. `bun run fetch` is
what populates `vendor/`, and what the release staging step reads.
...
different WGSL acceptance — and it happens even if not one line of this binding changed.
...
exact sha256 — [`wgpu-native.manifest.ts`](../wgpu-native.manifest.ts) for upstream's archives,
[`shim.manifest.ts`](../shim.manifest.ts) for the ABI shim — and a mismatch always hard-fails,
including under `--soft`, which exists so a fresh clone without network can proceed, not to wave
through an unexpected binary.

Every shim `sha256` is empty because no shim release has been cut. An empty hash means unpinned, and
`bun run shim:fetch` refuses to install one rather than invent a plausible-looking hash. That refusal
is maintainer-path only: consumers get the shim from the platform npm package.
...
surfaces as "not found in archive" rather than a silently-empty directory.

Bumping the pin is a two-line edit (`WGPU_NATIVE_TAG` + the URLs), then `bun run fetch:hashes` to
re-measure and `bun run gen:layouts` to regenerate the struct tables. Hashes are pasted by a human,
...
under this name. It says **nothing** about the origin of the bundled `wgpu_native` library, which is
covered separately by the pinned URL and sha256 in `wgpu-native.manifest.ts` — verifiable against
upstream's own release page.
