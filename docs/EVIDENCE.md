...
| **The whole suite against Dawn, on every platform** | 371 pass / 0 fail on each of the three Dawn platforms in CI, each reaching a real device: llvmpipe/Vulkan, WARP/D3D12, Apple Paravirtual/Metal. Same corpus as wgpu-native, run in the job that links the library |
...
| **Both backends, both implementations, on one machine** | `bun run test:matrix`: wgpu-native and Dawn × D3D12 and Vulkan, 376 pass / 0 fail in all four cells. CI cannot produce this — every runner has exactly one usable backend (WARP is D3D12-only, lavapipe Vulkan-only, macOS Metal) |
